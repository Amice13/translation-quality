...
write.csv(
  irr_results,
  file = result_path("Table 1 - IRR Results.csv"),
  row.names = FALSE
)
write.csv(
  duration_stats,
...
write.csv(
  irr,
  file = result_path("Table 3 - ICC Revised.csv"),
  row.names = FALSE
)
...
custom_ggsave(
  result_path("Figure 7 - Scatterplots for scores.png"),
  width = 180
)
...
plot_cor(cors)
custom_ggsave(
  result_path("Figure 8 - Text characteristics correlation matrix.png"),
  width = 220
)
